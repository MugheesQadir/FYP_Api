...
#--------------- Place -----------------
        ...
        try:
            ...
            if result is not None:
                return [{"home_id": home.id, "home_name":
                    home.name, "person_name": person.name,"Threshold":home.Threshold
    ...
    @staticmethod
    def update_home_Threshold(data):
        try:
            homes = Home.query.filter_by(id=data['id'],validate=1).first()
            if homes is None:
                return {'error':'Home not found'}
            ...
            if homes is not None:
                homes.Threshold = data['Threshold']
                db.session.commit()
                return {'success':f"Threshold updated successfully"}
        except Exception as a:
            return str(a)


    @staticmethod
    def update_home_no_of_rooms_app(data):
        try:
            homes = Home.query.filter_by(id=data['id'],validate=1).first()
            if homes is None:
                return {'error':'Home not found'}
            ...
            if homes is not None:
                homes.no_of_rooms_app = data['no_of_rooms_app']
                db.session.commit()
                return {'success':f"no_of_rooms_app updated successfully"}
        except Exception as a:
            return str(a)
