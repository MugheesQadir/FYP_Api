...
@app.route('/update_home_Threshold',methods = ["POST"])
def update_home_Threshold():
    data = request.get_json()
    home = HomeController.update_home_Threshold(data)
    return jsonify(home)

@app.route('/update_home_no_of_rooms_app',methods = ["POST"])
def update_home_no_of_rooms_app():
    data = request.get_json()
    home = HomeController.update_home_no_of_rooms_app(data)
    return jsonify(home)
...
@app.route('/list_compartment_appliance_logs_by_compartment_id/<int:id>',methods=['GET'])
...
@app.route('/get_appliancess_with_active_appliances_by_home_id/<int:home_id>',methods = ['GET'])
def get_appliancess_with_active_appliances_by_home_id(home_id):
    compartments = FYPTaskController.get_appliancess_with_active_appliances_by_home_id(home_id)
    return jsonify(compartments)

@app.route('/get_appliancess_with_active_appliances_geyser_by_home_id/<int:home_id>',methods = ['GET'])
def get_appliancess_with_active_appliances_geyser_by_home_id(home_id):
    compartments = FYPTaskController.get_appliancess_with_active_appliances_geyser_by_home_id(home_id)
    return jsonify(compartments)

@app.route('/get_Appliances_On_by_Home_id/<int:home_id>/<int:watt>',methods = ['GET'])
def get_Appliances_On_by_Home_id(home_id,watt):
    compartments = FYPTaskController.get_Appliances_On_by_Home_id(home_id,watt)
    return jsonify(compartments)


@app.route('/Lobby_Light_always_On/<int:home_id>',methods = ['GET'])
def Lobby_Light_always_On(home_id):
    compartments = FYPTaskController.Lobby_Light_always_On(home_id)
    return jsonify(compartments)

@app.route('/turn_off_all_appliances_at_night_by_home_id', methods=['GET'])
def turn_off_all_appliances_at_night_by_home_id():
    result = FYPTaskController.turn_off_all_appliances_at_night_by_home_id()
    return jsonify(result) ,200

@app.route('/get_watt_with_compartment_id/<int:comp_id>',methods = ["GET"])
def get_watt_with_compartment_id(comp_id):
    appliance = FYPTaskController.get_watt_with_compartment_id(comp_id)
    return jsonify(appliance)


#-------------------- Threads ------------------

def bg(requests=None):
    time.sleep(1)
    while True:
        try:
            print("")
            # app = requests.get("http;//127.0.01:5000/check_schedule_update_status")
            # try:
            #     app_data = app.json()
            # except Exception as e:
            #     print("failed",e)
        ...
        #     print("")
        #     app = requests.get("http;//10.108.181.116:5000/turn_off_all_appliances_at_night_by_home_id")
        #     try:
        #         app_data = app.json()
        #     except Exception as e:
        #         print("failed", e)
